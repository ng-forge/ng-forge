---
name: ng-forge-dynamic-forms-primeng
description: PrimeNG specific field properties for @ng-forge/dynamic-forms. Use alongside the ng-forge-dynamic-forms skill when the project depends on @ng-forge/dynamic-forms-primeng.
---

<!-- Generated by scripts/generate-skills.ts. Do not edit by hand. -->

# ng-forge Dynamic Forms: PrimeNG

Adapter specific `props` for `@ng-forge/dynamic-forms-primeng`.

This skill documents version **1.2.0**.

## Use with the core skill

This skill covers only what PrimeNG adds. Field types, validation, conditional
logic and the authoring rules are the same for every adapter and live in the
`ng-forge-dynamic-forms` skill. Read that one first.

## Confirm the project uses this adapter

```bash
node -e "console.log(require('@ng-forge/dynamic-forms-primeng/package.json').version)"
```

If that fails, the project uses a different adapter and none of the properties
below apply. Two adapters cannot both provide the same field type: TypeScript
reports error 2717 when they try, so a project has one adapter per field type.

Fields are registered with `withPrimeNGFields()`.

## Validate against this adapter

```bash
npx --yes @ng-forge/dynamic-forms-cli@1.2.0 "path/to/your.form.ts" --ui primeng
```

Passing the wrong `--ui` validates against a different adapter's properties, which
can accept a config this project will not render. The version is pinned to
`1.2.0` so the validator matches the properties listed here. Requires
Node 24 or newer.

## Reference

- [Adapter properties](references/props.md) — every `props` key PrimeNG adds, per field type
