...
test.describe('Comprehensive PrimeNG Field Tests', () => {
  ...
  test.describe('Comprehensive Fields Test', () => {
    test('should test all basic field types', async ({ page, helpers }) => {
      ...
      // Test Select Field (PrimeNG p-select)
      await expect(scenario.locator('#selectField p-select')).toBeVisible();
      await scenario.locator('#selectField p-select').click();
      // Wait for overlay to appear and click option
      await page.locator('.p-select-option').filter({ hasText: 'Option 2' }).click();

      // Test Radio Field
      await expect(scenario.locator('#radioField p-radiobutton')).toBeVisible();
      await scenario.locator('#radioField').getByText('Radio Option 2').click();

      // Test Checkbox Field
      await expect(scenario.locator('#checkboxField p-checkbox')).toBeVisible();
      await scenario.locator('#checkboxField p-checkbox').click();

      // Test Toggle Field
      await expect(scenario.locator('#toggleField p-toggleswitch')).toBeVisible();
      ...
      await scenario.locator('#multiCheckboxField').getByText('Checkbox 1').click();
      await scenario.locator('#multiCheckboxField').getByText('Checkbox 3').click();

      // Test Datepicker Field (PrimeNG p-datepicker)
      await expect(scenario.locator('#datepickerField p-datepicker')).toBeVisible();
      await scenario.locator('#datepickerField p-datepicker button').click();
      // Wait for calendar overlay and select today's date
      await page.locator('.p-datepicker-today').click();

      // Test Slider Field
      await expect(scenario.locator('#sliderField p-slider')).toBeVisible();
      // Move slider using keyboard (PrimeNG slider responds to arrow keys)
  ...
  test.describe('State Management Test', () => {
    test('should test form state management', async ({ page, helpers }) => {
      ...
      // Toggle checkbox (PrimeNG p-checkbox)
      await scenario.locator('#stateCheckbox p-checkbox').click();
