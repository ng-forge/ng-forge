...
test.describe('Multi-Page Navigation Tests', () => {
  ...
  test.describe('Multi-Page Registration Wizard', () => {
    test('should complete full 3-page registration workflow', async ({ page, helpers }) => {
      ...
      await expect(scenario.locator('#newsletter p-checkbox')).toBeVisible();
      ...
      await scenario.locator('#language p-select').click();
      await page.locator('.p-select-option:has-text("English")').click();
      await page.waitForTimeout(200);

      // Select timezone
      await scenario.locator('#timezone p-select').click();
      await page.locator('.p-select-option:has-text("Eastern Time")').click();
      await page.waitForTimeout(200);

      // Agree to terms (required)
      await scenario.locator('#terms p-checkbox').click();
      await page.waitForTimeout(200);
