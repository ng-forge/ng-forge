...
test.describe('User Workflows E2E Tests', () => {
  ...
  test.describe('User Registration Workflow', () => {
    test('should complete full registration workflow', async ({ page, helpers }) => {
      ...
      // Agree to terms - PrimeNG checkbox
      await page.waitForSelector('[data-testid="registration-workflow"] #agreeTerms p-checkbox input', {
        state: 'visible',
        timeout: 10000,
      });
      const agreeTermsCheckbox = scenario.locator('#agreeTerms p-checkbox input');
      ...
      await page.waitForSelector('[data-testid="registration-workflow"] #submit button:not([disabled])', {
        state: 'visible',
        timeout: 15000,
      });
  ...
  test.describe('Profile Edit Workflow', () => {
    test('should edit and save user profile', async ({ page, helpers }) => {
      ...
      await page.waitForSelector('[data-testid="profile-edit"] #submit button:not([disabled])', { state: 'visible', timeout: 10000 });
      await expect(submitButton).toBeEnabled({ timeout: 10000 });
  ...
  test.describe('Multi-Field Validation Workflow', () => {
    test('should validate required fields before allowing submission', async ({ page, helpers }) => {
      ...
      // Wait for submit button to be enabled
      await page.waitForSelector('[data-testid="validation-workflow"] #submit button:not([disabled])', {
        state: 'visible',
        timeout: 10000,
      });
  ...
  test.describe('Form Reset Workflow', () => {
    test('should reset form after modification', async ({ page, helpers }) => {
      ...
      const activeCheckbox = scenario.locator('#active p-checkbox input');
      ...
      await page.waitForSelector('[data-testid="reset-workflow"] #active p-checkbox input', { state: 'visible', timeout: 10000 });
  ...
  test.describe('Complex Form Workflow', () => {
    test('should complete contact form', async ({ page, helpers }) => {
      ...
      // Wait for submit button to be enabled
      await page.waitForSelector('[data-testid="contact-form"] #submit button:not([disabled])', { state: 'visible', timeout: 10000 });
