...
/**
 * Extended test context with common page helpers for PrimeNG
 */
  ...
  helpers: async ({ page }, use) => {
    const helpers: TestHelpers = {
      ...
      getFieldError: (scenario: Locator, fieldId: string) => {
        return scenario.locator(`#${fieldId} .p-error`).first();
      },

      getFieldErrors: (scenario: Locator, fieldId: string) => {
        return scenario.locator(`#${fieldId} .p-error`);
      },
      ...
      selectOption: async (select: Locator, optionText: string) => {
        ...
        await select.click();
        // Wait for dropdown overlay to appear and select the option
      ...
      selectRadio: async (scenario: Locator, fieldId: string, labelText: string) => {
        // PrimeNG radio buttons structure: each option has a label with text
        // We find the field container, then click the label with the matching text
      ...
      fillDatepicker: async (scenario: Locator, fieldId: string, value: string) => {
        // PrimeNG datepicker: clear input, type the date, then blur to trigger validation
        const input = scenario.locator(`#${fieldId} input`);
        // Clear any existing value and type the new date
        await input.clear();
        await input.pressSequentially(value, { delay: 20 });
        // Press Tab to move focus away and trigger the change event
      ...
      expectErrorVisible: async (scenario: Locator, fieldId: string, options?: { timeout?: number }) => {
        ...
        const errorLocator = scenario.locator(`#${fieldId} .p-error`).first();
      ...
      expectNoErrorVisible: async (scenario: Locator, fieldId: string, options?: { timeout?: number }) => {
        const timeout = options?.timeout ?? 5000;
        const errorLocator = scenario.locator(`#${fieldId} .p-error`);
