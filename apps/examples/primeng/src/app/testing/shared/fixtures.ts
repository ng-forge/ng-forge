...
export interface TestHelpers {
  ...
  /** Get the error message for a field (small.p-error element) */
  ...
  helpers: async ({ page }, use) => {
    const helpers: TestHelpers = {
      ...
      getFieldError: (scenario: Locator, fieldId: string) => {
        return scenario.locator(`#${fieldId} small.p-error`).first();
      },

      getFieldErrors: (scenario: Locator, fieldId: string) => {
        return scenario.locator(`#${fieldId} small.p-error`);
      },
      ...
      selectOption: async (select: Locator, optionText: string) => {
        await select.click();
        await page.waitForSelector('.p-select-overlay', { state: 'visible' });
        await page.locator('.p-select-overlay .p-select-option').filter({ hasText: optionText }).click();
      },
