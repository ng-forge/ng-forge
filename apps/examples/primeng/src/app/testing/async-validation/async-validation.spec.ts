...
test.describe('Async Validation Tests', () => {
  ...
  test.describe('HTTP GET Validator', () => {
    test('should validate username availability using HTTP GET validator', async ({ page, helpers, browserName }) => {
      ...
      await page.goto('http://localhost:4202/#/test/async-validation/http-get-validator');
      ...
      const errorMessage = usernameField.locator('.p-error:has-text("already taken")');
  ...
  test.describe('HTTP POST Validator', () => {
    test('should validate email using HTTP POST validator', async ({ page, helpers, browserName }) => {
      ...
      await page.goto('http://localhost:4202/#/test/async-validation/http-post-validator');
      await page.waitForLoadState('networkidle');
      await expect(scenario).toBeVisible({ timeout: 10000 });
      ...
      const errorMessage = emailField.locator('.p-error:has-text("not valid")');
  ...
  test.describe('Resource-Based Async Validator', () => {
    test('should validate product code using resource-based async validator', async ({ page, helpers }) => {
      ...
      await page.goto('http://localhost:4202/#/test/async-validation/async-resource-validator');
      await page.waitForLoadState('networkidle');
      await expect(scenario).toBeVisible({ timeout: 10000 });
      ...
      const errorVisible = await productCodeField
        .locator('.p-error:has-text("already in use")')
        .isVisible()
        .catch(() => false);
  ...
  test.describe('HTTP Error Handling', () => {
    test('should handle HTTP validation network errors gracefully', async ({ page, helpers }) => {
      ...
      await page.goto('http://localhost:4202/#/test/async-validation/http-error-handling');
      await page.waitForLoadState('networkidle');
      await expect(scenario).toBeVisible({ timeout: 10000 });
      ...
      const usernameField = scenario.locator('#username');
      const errorMessage = usernameField.locator('.p-error');
  ...
  test.describe('Multiple Validators', () => {
    test('should validate multiple async validators on same field', async ({ page, helpers }) => {
      ...
      await page.goto('http://localhost:4202/#/test/async-validation/multiple-validators');
      await page.waitForLoadState('networkidle');
      await expect(scenario).toBeVisible({ timeout: 10000 });
      ...
      const usernameField = scenario.locator('#username');
      const asyncErrorMessage = usernameField.locator('.p-error:has-text("already taken")');
      ...
      const errorMessages = usernameField.locator('.p-error');
