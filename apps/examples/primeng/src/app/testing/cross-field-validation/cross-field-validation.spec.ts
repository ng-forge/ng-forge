...
test.describe('Cross-Field Validation Tests', () => {
  ...
  test.describe('Password Confirmation Validation', () => {
    test('should validate password matching', async ({ page, helpers }) => {
      ...
      await page.waitForSelector('[data-testid="password-validation"] #submitPassword button:not([disabled])', {
        state: 'visible',
        timeout: 10000,
      });
  ...
  test.describe('Conditional Required Fields', () => {
    test('should handle conditional field requirements', async ({ page, helpers }) => {
      ...
      await page.waitForSelector('[data-testid="conditional-validation"] #hasAddress p-checkbox input', {
        state: 'visible',
        timeout: 10000,
      });
  ...
  test.describe('Dependent Select Fields', () => {
    test('should handle dependent field selections', async ({ page, helpers }) => {
      ...
      await page.waitForSelector('[data-testid="dependent-fields"] #category p-select', { state: 'visible', timeout: 10000 });
      ...
      // Wait for submit button to be enabled
      await page.waitForSelector('[data-testid="dependent-fields"] #submitDependent button:not([disabled])', {
        state: 'visible',
        timeout: 10000,
      });
  ...
  test.describe('Field Enable/Disable', () => {
    test('should handle field enabling/disabling based on selections', async ({ page, helpers }) => {
      ...
      await shippingAddressTextarea.blur();

      // Wait for submit button to be enabled
      await page.waitForSelector('[data-testid="enable-disable"] #submitEnableDisable button:not([disabled])', {
        state: 'visible',
        timeout: 10000,
      });
      ...
      await expressInstructionsTextarea.blur();
      ...
      await page.waitForSelector('[data-testid="enable-disable"] #storeLocation p-select', { state: 'visible', timeout: 10000 });

      // Select pickup location
      await helpers.selectOption(scenario.locator('#storeLocation p-select'), 'Downtown Store');
