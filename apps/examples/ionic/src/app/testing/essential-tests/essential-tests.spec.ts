...
test.describe('Essential Tests - Quick Validation', () => {
  test('basic form functionality works', async ({ helpers, page }) => {
    await helpers.navigateToScenario('/testing/essential-tests/basic-form');
    ...
    // Ionic ion-button uses aria-disabled attribute for disabled state (not native disabled for Playwright)
    ...
    await page.waitForSelector('[data-testid="basic-form"] #submit ion-button:not([aria-disabled="true"])', {
      state: 'visible',
      timeout: 10000,
    });
  ...
  test('age-based logic works correctly', async ({ helpers, page }) => {
    await helpers.navigateToScenario('/testing/essential-tests/age-based-logic');
  ...
  test('multi-page navigation works', async ({ helpers, page }) => {
    await helpers.navigateToScenario('/testing/essential-tests/multi-page-navigation');
    ...
    // Wait for button to become enabled
    await page.waitForSelector('[data-testid="multi-page-navigation"] #submit ion-button:not([aria-disabled="true"])', {
      state: 'visible',
      timeout: 10000,
    });
    await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
