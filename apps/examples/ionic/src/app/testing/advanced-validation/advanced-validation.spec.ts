...
test.describe('Advanced Validation E2E Tests', () => {
  test.beforeEach(async ({ helpers }) => {
    await helpers.navigateToScenario('/testing/advanced-validation');
  });
  ...
  test.describe('Custom Validator', () => {
    test('should validate password strength using custom validator', async ({ page, helpers }) => {
      ...
      await page.waitForSelector('[data-testid="custom-validator-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
  ...
  test.describe('Cross-Field Validation', () => {
    test('should validate password matching across fields', async ({ page, helpers }) => {
      ...
      // Wait for form to become valid
      await page.waitForSelector('[data-testid="cross-field-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
  ...
  test.describe('Range Validation', () => {
    test('should validate that maximum is greater than minimum', async ({ page, helpers }) => {
      ...
      await ionBlur(minInput);

      await maxInput.fill('20');
      await expect(maxInput).toHaveValue('20', { timeout: 5000 });
      await ionBlur(maxInput);

      // Wait for form to become valid
      await page.waitForSelector('[data-testid="range-validation-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
  ...
  test.describe('Conditional Validation', () => {
    test('should apply validators conditionally based on field values', async ({ page, helpers }) => {
      ...
      // Wait for inputs to be visible
      await page.waitForSelector('[data-testid="conditional-validator-test"] #isAdult ion-checkbox', {
        state: 'visible',
        timeout: 10000,
      });
      await page.waitForSelector('[data-testid="conditional-validator-test"] #age input', { state: 'visible', timeout: 10000 });

      const isAdultCheckbox = scenario.locator('#isAdult ion-checkbox');
      ...
      await helpers.checkIonCheckbox(isAdultCheckbox);
      ...
      // Wait for form to become valid
      await page.waitForSelector('[data-testid="conditional-validator-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
      ...
      await helpers.uncheckIonCheckbox(isAdultCheckbox);
  ...
  test.describe('Multiple Validators', () => {
    test('should apply multiple validators to a single field', async ({ page, helpers }) => {
      ...
      // Wait for form to become valid
      await page.waitForSelector('[data-testid="multiple-validators-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
  ...
  test.describe('Expression-Based Min/Max Validators', () => {
    test('should validate age against dynamic minAge value', async ({ page, helpers }) => {
      await page.goto(testUrl('/advanced-validation/expression-based-min-max'));
      ...
      await page.waitForSelector('[data-testid="expression-based-min-max-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
  ...
  test.describe('Cross-Field Error Targeting', () => {
    test('should validate password confirmation matches password', async ({ page, helpers }) => {
      await page.goto(testUrl('/advanced-validation/cross-field-error-targeting'));
      await page.waitForLoadState('networkidle');
      ...
      await ionBlur(targetValueInput);

      // Wait for form to become valid
      await page.waitForSelector('[data-testid="cross-field-error-targeting-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
      ...
      // Wait for error message to exist in DOM (Ionic CSS may hide error slot)
      await page.waitForSelector('[data-testid="cross-field-error-targeting-test"] #confirmPassword ion-note[color="danger"]', {
        state: 'attached',
        timeout: 10000,
      });
      // Verify validation state via aria-invalid
      const confirmPasswordIonInput = scenario.locator('#confirmPassword ion-input');
      await expect(confirmPasswordIonInput).toHaveAttribute('aria-invalid', 'true', { timeout: 5000 });
    ...
    test('should display correct error message for age validation', async ({ page, helpers }) => {
      ...
      // Wait for error message to exist in DOM (Ionic CSS may hide error slot)
      await page.waitForSelector('[data-testid="cross-field-error-targeting-test"] #age ion-note[color="danger"]', {
        state: 'attached',
        timeout: 10000,
      });

      // Verify validation state via aria-invalid
      const ageIonInput = scenario.locator('#age ion-input');
      await expect(ageIonInput).toHaveAttribute('aria-invalid', 'true', { timeout: 5000 });
      // Verify error message text
    ...
    test('should display correct error message for range validation', async ({ page, helpers }) => {
      ...
      // Wait for error message to exist in DOM (Ionic CSS may hide error slot)
      await page.waitForSelector('[data-testid="cross-field-error-targeting-test"] #targetValue ion-note[color="danger"]', {
        state: 'attached',
        timeout: 10000,
      });

      // Verify validation state via aria-invalid
      const targetValueIonInput = scenario.locator('#targetValue ion-input');
      await expect(targetValueIonInput).toHaveAttribute('aria-invalid', 'true', { timeout: 5000 });
      // Verify error message text
  ...
  test.describe('When Clause with AND/OR Logic', () => {
    test('should require company name only for verified business accounts', async ({ page, helpers }) => {
      await page.goto(testUrl('/advanced-validation/when-with-and-or'));
      await page.waitForLoadState('networkidle');
      ...
      await page.waitForSelector('[data-testid="when-with-and-or-test"] #isVerified ion-checkbox', {
        state: 'visible',
        timeout: 10000,
      });
      ...
      await ionBlur(companyNameInput);

      await page.waitForSelector('[data-testid="when-with-and-or-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
    ...
    test('should require payment method for premium OR discounted orders', async ({ page, helpers }) => {
      ...
      // Wait for inputs to be visible
      await page.waitForSelector('[data-testid="when-with-and-or-test"] #isPremium ion-checkbox', {
        state: 'visible',
        timeout: 10000,
      });
      await page.waitForSelector('[data-testid="when-with-and-or-test"] #hasDiscount ion-checkbox', {
        state: 'visible',
        timeout: 10000,
      });
      await page.waitForSelector('[data-testid="when-with-and-or-test"] #paymentMethod input', { state: 'visible', timeout: 10000 });

      const isPremiumCheckbox = scenario.locator('#isPremium ion-checkbox');
      ...
      await ionBlur(paymentMethodInput);
      ...
      await helpers.uncheckIonCheckbox(isPremiumCheckbox);
    ...
    test('should apply min validator for business OR verified premium accounts', async ({ page, helpers }) => {
      ...
      await ionBlur(orderAmountInput);
      await expect(submitButton).toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
  ...
  test.describe('Nested Field Paths', () => {
    test('should validate with nested field paths', async ({ page, helpers }) => {
      await page.goto(testUrl('/advanced-validation/nested-field-paths'));
      await page.waitForLoadState('networkidle');
    ...
    test('should hide shipping zip field when sameAsBilling is checked', async ({ page, helpers }) => {
      ...
      // Wait for inputs to be visible
      await page.waitForSelector('[data-testid="nested-field-paths-test"] #sameAsBilling ion-checkbox', {
        state: 'visible',
        timeout: 10000,
      });
      await page.waitForSelector('[data-testid="nested-field-paths-test"] #shippingZip', { state: 'visible', timeout: 10000 });

      const sameAsBillingCheckbox = scenario.locator('#sameAsBilling ion-checkbox');
      ...
      await helpers.checkIonCheckbox(sameAsBillingCheckbox);
      ...
      await helpers.uncheckIonCheckbox(sameAsBillingCheckbox);
    ...
    test('should validate shipping zip matches billing zip via nested path', async ({ page, helpers }) => {
      ...
      await ionBlur(shippingZipInput);

      // Form should be invalid due to zip mismatch
      await expect(submitButton).toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });

      // Wait for error message to exist in DOM (Ionic CSS may hide error slot)
      await page.waitForSelector('[data-testid="nested-field-paths-test"] #shippingZip ion-note[color="danger"]', {
        state: 'attached',
        timeout: 10000,
      });

      // Verify validation state via aria-invalid
      const shippingZipIonInput = scenario.locator('#shippingZip ion-input');
      await expect(shippingZipIonInput).toHaveAttribute('aria-invalid', 'true', { timeout: 5000 });
      ...
      await page.waitForSelector('[data-testid="nested-field-paths-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      await expect(submitButton).not.toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });
    ...
    test('should validate secondary email differs from nested primary email', async ({ page, helpers }) => {
      ...
      await ionBlur(secondaryEmailInput);

      // Form should be invalid due to duplicate email
      await expect(submitButton).toHaveAttribute('aria-disabled', 'true', { timeout: 10000 });

      // Wait for error message to exist in DOM (Ionic CSS may hide error slot)
      await page.waitForSelector('[data-testid="nested-field-paths-test"] #secondaryEmail ion-note[color="danger"]', {
        state: 'attached',
        timeout: 10000,
      });

      // Verify validation state via aria-invalid
      const secondaryEmailIonInput = scenario.locator('#secondaryEmail ion-input');
      await expect(secondaryEmailIonInput).toHaveAttribute('aria-invalid', 'true', { timeout: 5000 });
      // Verify error message text
    ...
    test('should verify SMS notifications checkbox and phone field exist', async ({ page, helpers }) => {
      ...
      const smsCheckbox = scenario.locator('#smsNotifications ion-checkbox');
      ...
      // Check SMS checkbox
      await helpers.checkIonCheckbox(smsCheckbox);
  ...
  test.describe('Array Cross-Validation', () => {
    test('should validate array item with require email checkbox', async ({ page, helpers }) => {
      await page.goto(testUrl('/advanced-validation/array-cross-validation'));
      await page.waitForLoadState('networkidle');
      ...
      const requireEmailCheckbox = scenario.locator('#requireEmail ion-checkbox');
      const submitButton = helpers.getSubmitButton(scenario);
      ...
      await page.waitForSelector('[data-testid="array-cross-validation-test"] #submit ion-button:not([aria-disabled="true"])', {
        state: 'visible',
        timeout: 10000,
      });
      ...
      await helpers.checkIonCheckbox(requireEmailCheckbox);
    ...
    test('should display correct error message for email validation in array item', async ({ page, helpers }) => {
      ...
      await page.waitForSelector('[data-testid="array-cross-validation-test"] #requireEmail ion-checkbox', {
        state: 'visible',
        timeout: 10000,
      });
      const requireEmailCheckbox = scenario.locator('#requireEmail ion-checkbox');
      ...
      // Wait for error message to exist in DOM (Ionic CSS may hide error slot)
      await page.waitForSelector('[data-testid="array-cross-validation-test"] ion-note[color="danger"]:has-text("valid email")', {
        state: 'attached',
        timeout: 20000,
      });
      const errorMessage = scenario.locator('ion-note[color="danger"]').filter({ hasText: 'valid email' });
      // Verify error exists and has expected text
      await expect(errorMessage).toHaveCount(1, { timeout: 10000 });
