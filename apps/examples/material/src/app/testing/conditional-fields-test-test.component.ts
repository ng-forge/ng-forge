import { Component, signal } from '@angular/core';
import { JsonPipe } from '@angular/common';
import { DynamicForm } from '@ng-forge/dynamic-form';

/**
 * Conditional Fields Test Component
 * Dedicated component with embedded test configs for conditional fields e2e tests
 */
@Component({
  selector: 'app-conditional-fields-test-test',
  imports: [DynamicForm, JsonPipe],
  template: `
    <div class="test-page">
      <h1>Conditional Fields Tests</h1>

      <!-- Test scenarios defined directly in component -->
      @for (scenario of scenarios; track scenario.testId) {
        <section class="test-scenario" [attr.data-testid]="scenario.testId">
          <h2>{{ scenario.title }}</h2>
          <dynamic-form [config]="scenario.config" [(value)]="scenario.value" (submitted)="onSubmitted($event, scenario.testId)" />

          <details class="debug-output">
            <summary>Debug Output</summary>
            <pre [attr.data-testid]="'form-value-' + scenario.testId">{{ scenario.value() | json }}</pre>
          </details>
        </section>
      }
    </div>
  `,
  styles: [
    `
      .test-page {
        padding: 2rem;
        max-width: 1200px;
        margin: 0 auto;
      }

      .test-scenario {
        margin-bottom: 3rem;
        padding: 2rem;
        border: 1px solid #e0e0e0;
        border-radius: 8px;
        background: #fafafa;
      }

      .test-scenario h2 {
        margin-top: 0;
        color: #1976d2;
      }

      .debug-output {
        margin-top: 2rem;
        padding: 1rem;
        border: 1px solid #ddd;
        border-radius: 4px;
        background: #fff;
      }

      pre {
        background: #f5f5f5;
        padding: 1rem;
        border-radius: 4px;
        overflow-x: auto;
      }
    `,
  ],
})
export class ConditionalFieldsTestTestComponent {
  scenarios = [
    {
      testId: 'conditional-basic',
      title: 'Basic Conditional Fields',
      config: {
        fields: [
          {
            key: 'hasAccount',
            type: 'radio',
            label: 'Do you have an existing account?',
            required: true,
            options: [
              { value: 'yes', label: 'Yes, I have an account' },
              { value: 'no', label: 'No, I need to create one' },
            ],
            props: { color: 'primary' },
          },
          // Fields shown when user has an account
          {
            key: 'loginEmail',
            type: 'input',
            label: 'Login Email',
            logic: [
              {
                type: 'hidden',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'notEquals',
                  value: 'yes',
                },
              },
              {
                type: 'required',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'equals',
                  value: 'yes',
                },
                errorMessage: 'Login email is required',
              },
            ],
            props: {
              type: 'email',
              appearance: 'outline',
            },
          },
          {
            key: 'password',
            type: 'input',
            label: 'Password',
            logic: [
              {
                type: 'hidden',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'notEquals',
                  value: 'yes',
                },
              },
              {
                type: 'required',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'equals',
                  value: 'yes',
                },
                errorMessage: 'Password is required',
              },
            ],
            props: {
              type: 'password',
              appearance: 'outline',
            },
          },
          // Fields shown when user needs to create an account
          {
            key: 'firstName',
            type: 'input',
            label: 'First Name',
            logic: [
              {
                type: 'hidden',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'notEquals',
                  value: 'no',
                },
              },
              {
                type: 'required',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'equals',
                  value: 'no',
                },
                errorMessage: 'First name is required',
              },
            ],
            props: {
              appearance: 'outline',
            },
          },
          {
            key: 'lastName',
            type: 'input',
            label: 'Last Name',
            logic: [
              {
                type: 'hidden',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'notEquals',
                  value: 'no',
                },
              },
              {
                type: 'required',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'equals',
                  value: 'no',
                },
                errorMessage: 'Last name is required',
              },
            ],
            props: {
              appearance: 'outline',
            },
          },
          {
            key: 'emailAddress',
            type: 'input',
            label: 'Email Address',
            logic: [
              {
                type: 'hidden',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'notEquals',
                  value: 'no',
                },
              },
              {
                type: 'required',
                condition: {
                  type: 'fieldValue',
                  fieldPath: 'hasAccount',
                  operator: 'equals',
                  value: 'no',
                },
                errorMessage: 'Email address is required',
              },
            ],
            props: {
              type: 'email',
              appearance: 'outline',
            },
          },
        ],
      },
      value: signal<Record<string, unknown>>({}),
    },
  ];

  submissionLog = signal<Array<{ timestamp: string; testId: string; data: Record<string, unknown> }>>([]);

  onSubmitted(value: Record<string, unknown> | undefined, testId: string): void {
    if (!value) return;

    const submission = {
      timestamp: new Date().toISOString(),
      testId,
      data: value,
    };

    this.submissionLog.update((log) => [...log, submission]);

    window.dispatchEvent(new CustomEvent('formSubmitted', { detail: submission }));
  }
}
