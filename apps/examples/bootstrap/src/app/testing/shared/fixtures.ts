...
/**
 * Extended test context with common page helpers for Bootstrap
 */
  ...
  helpers: async ({ page }, use) => {
    const helpers: TestHelpers = {
      ...
      getFieldError: (scenario: Locator, fieldId: string) => {
        return scenario.locator(`#${fieldId} .invalid-feedback`).first();
      },

      getFieldErrors: (scenario: Locator, fieldId: string) => {
        return scenario.locator(`#${fieldId} .invalid-feedback`);
      },
      ...
      selectOption: async (select: Locator, optionText: string) => {
        ...
        await select.blur(); // Trigger validation
      },
