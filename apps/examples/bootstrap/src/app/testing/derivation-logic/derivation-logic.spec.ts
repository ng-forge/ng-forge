...
import { testUrl } from '../shared/fixtures';
...
test.describe('Value Derivation Logic Tests', () => {
  ...
  test.describe('Function-Based Derivation', () => {
    ...
    test('should calculate discounted price using custom function', async ({ page, helpers }) => {
      ...
      const discountCodeSelect = scenario.locator('#discountCode select');
      ...
      // Apply SAVE10 (10% off)
      await discountCodeSelect.selectOption({ label: 'SAVE10 (10% off)' });
      await page.waitForTimeout(500);

      await expect(discountedPriceInput).toHaveValue('90');

      // Apply SAVE25 (25% off)
      await discountCodeSelect.selectOption({ label: 'SAVE25 (25% off)' });
      await page.waitForTimeout(500);

      await expect(discountedPriceInput).toHaveValue('75');

      // Apply HALF (50% off)
      await discountCodeSelect.selectOption({ label: 'HALF (50% off)' });
      await page.waitForTimeout(500);
  ...
  test.describe('Conditional Derivation', () => {
    ...
    test('should derive discount percentage and free shipping based on membership type', async ({ page, helpers }) => {
      ...
      const membershipSelect = scenario.locator('#membershipType select');
      const discountInput = helpers.getInput(scenario, 'discountPercent');
      // Bootstrap toggle uses a checkbox input with form-check-input class
      ...
      // Select Premium
      await membershipSelect.selectOption({ label: 'Premium' });
      await page.waitForTimeout(500);
      ...
      // Select VIP
      await membershipSelect.selectOption({ label: 'VIP' });
      await page.waitForTimeout(500);
      ...
      // Back to Basic
      await membershipSelect.selectOption({ label: 'Basic' });
      await page.waitForTimeout(500);
