...
test.describe('Error Handling and Edge Cases', () => {
  ...
  test.describe('Invalid Configuration Handling', () => {
    test('should handle invalid field configurations gracefully', async ({ page, helpers }) => {
      ...
      await page.goto('/#/primeng/test/error-handling/invalid-config');
      ...
      await page.waitForSelector('[data-testid="invalid-config"] #submitInvalid button:not([disabled])', {
        state: 'visible',
        timeout: 10000,
      });
  ...
  test.describe('Basic Form Functionality', () => {
    test('should handle form submission without errors', async ({ page, helpers }) => {
      ...
      await page.goto('/#/primeng/test/error-handling/basic-test');
      ...
      // Select radio option - PrimeNG uses p-radiobutton
      const radioInput = scenario.locator('#priority p-radiobutton input').first();
      await radioInput.check();
      await expect(radioInput).toBeChecked({ timeout: 5000 });

      // Wait for button to be enabled
      await page.waitForSelector('[data-testid="basic-test"] #submit button:not([disabled])', { state: 'visible', timeout: 10000 });
    ...
    test('should handle rapid form interactions without errors', async ({ page, helpers }) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // Quick radio selection - PrimeNG uses p-radiobutton
        const radioButtons = scenario.locator('#priority p-radiobutton input');
  ...
  test.describe('Form State Management', () => {
    test('should maintain form state during browser navigation', async ({ page, helpers }) => {
      ...
      await page.goto('/#/primeng/examples');
      await page.waitForLoadState('networkidle');
      await page.goto('/#/primeng/test/error-handling/basic-test');
      await page.waitForLoadState('networkidle');
