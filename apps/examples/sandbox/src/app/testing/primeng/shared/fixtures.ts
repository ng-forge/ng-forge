...
export const testUrl = (path: string) => `http://localhost:${APP_PORTS['sandbox-examples']}/#/primeng${path}`;
...
export const test = base.extend<{ helpers: TestHelpers; consoleTracker: ConsoleTracker; mockApi: MockApiHelpers }>({
  ...
  page: async ({ page }, use) => {
    ...
    page.goto = (url: string, options?: Parameters<typeof page.goto>[1]) =>
      originalGoto(url.startsWith('/#/test/') ? `/#/primeng${url.slice(2)}` : url, options);
    await use(page);
  },
