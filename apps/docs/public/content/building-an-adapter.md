...
If you write a custom mapper that emits a key the consuming component doesn't declare, the form engine fails fast at runtime with a clear error pointing at the mismatch — there's no silent drop. So if you're authoring a new field category, treat the mapper output as the wire format and the component inputs as the schema.
