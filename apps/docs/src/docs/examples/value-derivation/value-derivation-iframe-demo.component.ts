...
export class ValueDerivationIframeDemoComponent {
  ...
  fields: [
    ...
    { key: 'subtotal', type: 'input', label: 'Subtotal ($)', value: 25, props: { type: 'number' }, readonly: true, col: 4 },
    { key: 'taxRate', type: 'input', label: 'Tax Rate (%)', value: 10, props: { type: 'number' }, col: 4,
      logic: [
        { type: 'derivation', targetField: 'tax', expression: 'formValue.subtotal * formValue.taxRate / 100' },
        { type: 'derivation', targetField: 'total', expression: 'formValue.subtotal + formValue.tax' }
      ] },
    { key: 'tax', type: 'input', label: 'Tax ($)', value: 2.5, props: { type: 'number' }, readonly: true, col: 4 },
    { key: 'total', type: 'input', label: 'Total ($)', value: 27.5, props: { type: 'number' }, readonly: true, col: 4 },
    ...
    { key: 'lastName', type: 'input', label: 'Last Name', value: 'Doe', col: 4,
      logic: [{ type: 'derivation', targetField: 'fullName', expression: '(formValue.firstName || "") + " " + (formValue.lastName || "")' }] },
    { key: 'fullName', type: 'input', label: 'Full Name', value: 'John Doe', readonly: true, col: 4 },
  ],
}`;
}
