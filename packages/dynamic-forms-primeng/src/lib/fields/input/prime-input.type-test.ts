...
describe('PrimeInputField - addons', () => {
  it('accepts prime-icon addon with required `icon`', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'prefix', type: 'prime-icon', icon: 'search' }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'prime-icon'>();
  });
  ...
  it('accepts prime-button addon with preset', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'suffix', type: 'prime-button', icon: 'times', ariaLabel: 'Clear', preset: 'clear' }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'prime-button'>();
  });
  ...
  it('accepts text addon with DynamicText', () => {
    const field = {
      type: 'input',
      key: 'amount',
      addons: [{ slot: 'prefix', type: 'text', text: '$' }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'text'>();
  });
  ...
  it('accepts template addon with templateKey', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'suffix', type: 'template', templateKey: 'mySlot' }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'template'>();
  });

  it("base addon types are present in PrimeInput's union", () => {
    // Base types always appear; user augmentations via `PrimeAddonExtensions`
    ...
    type AcceptedTypes = NonNullable<PrimeInputField['addons']>[number]['type'];
    expectTypeOf<'prime-icon' | 'prime-button' | 'text' | 'template'>().toMatchTypeOf<AcceptedTypes>();
  });
  ...
  it('PrimeAddonExtensions is empty by default — augmentable seam', () => {
    ...
    // bring custom types into the typed union.
  ...
  it('accepts addons with optional reactive hidden flag', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'suffix', type: 'prime-button', icon: 'x', ariaLabel: 'Clear', preset: 'clear', hidden: false }],
    } as const satisfies PrimeInputField;
  ...
  it('rejects icon-only prime-button without ariaLabel via content XOR', () => {
    ...
    type IconOnlyMissingAria = { slot: 'suffix'; type: 'prime-button'; icon: 'times'; preset: 'clear' };
    type AcceptedPiButtonShapes = Extract<NonNullable<PrimeInputField['addons']>[number], { type: 'prime-button' }>;
  ...
  it('accepts labeled prime-button without ariaLabel', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'suffix', type: 'prime-button', label: 'Clear', preset: 'clear' }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'prime-button'>();
  });
  ...
  it('accepts prime-button with actionRef alone (click XOR)', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'suffix', type: 'prime-button', label: 'Open', actionRef: 'openProfile' }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'prime-button'>();
  });
  ...
  it('accepts prime-button with inline action alone (click XOR)', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'suffix', type: 'prime-button', label: 'Run', action: () => undefined }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'prime-button'>();
  });
  ...
  it('accepts decorative prime-button with no click handler', () => {
    const field = {
      type: 'input',
      key: 'q',
      addons: [{ slot: 'suffix', type: 'prime-button', label: 'Static' }],
    } as const satisfies PrimeInputField;
    expectTypeOf(field.addons[0].type).toEqualTypeOf<'prime-button'>();
  });
