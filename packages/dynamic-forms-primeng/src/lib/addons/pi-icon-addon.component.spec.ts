...
import { PiIconAddonComponent } from './pi-icon-addon.component';

function setup(addon: PiIconAddon) {
  TestBed.configureTestingModule({ imports: [PiIconAddonComponent] });
  const fixture = TestBed.createComponent(PiIconAddonComponent);
  fixture.componentRef.setInput('addon', addon);
  fixture.detectChanges();
  return fixture;
}

describe('PiIconAddonComponent', () => {
  it('renders <i> with `pi pi-{icon}` class', () => {
    const fixture = setup({ kind: 'pi-icon', slot: 'prefix', icon: 'search' });
  ...
  it('forwards ariaLabel to the inner <i> and clears host aria-hidden', async () => {
    const fixture = setup({ kind: 'pi-icon', slot: 'prefix', icon: 'check', ariaLabel: 'Success' });
  ...
  it('rerenders the icon class when the `addon` input changes', () => {
    ...
    fixture.componentRef.setInput('addon', { kind: 'pi-icon', slot: 'prefix', icon: 'times' } satisfies PiIconAddon);
    fixture.detectChanges();
