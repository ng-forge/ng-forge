...
describe('MatContainerErrorsWrapperComponent', () => {
  ...
  const errorTexts = (fixture: ComponentFixture<Host>): string[] =>
    Array.from((fixture.nativeElement as HTMLElement).querySelectorAll('mat-error')).map((el) => el.textContent?.trim() ?? '');
  ...
  it('renders a mat-error with the resolved message once touched and invalid', async () => {
