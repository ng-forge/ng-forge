...
export interface PresetCollaborators {
  /** Per-field input-type override signal — populated only inside `bs-input`. */
 ...
 */
export async function runBsPresetAction(
  ...
  switch (preset) {
    ...
    case 'toggle-password-visibility': {
      ...
      if (!override) {
        collaborators.logger.warn(`preset 'toggle-password-visibility' has no effect outside a 'bs-input' field.`);
        return;
      }
    ...
    default:
      collaborators.logger.warn(`preset '${String(preset)}' is not implemented in the Bootstrap adapter.`);
  }
}
