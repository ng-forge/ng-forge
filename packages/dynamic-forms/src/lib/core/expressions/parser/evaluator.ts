...
export class Evaluator {
  ...
  evaluate(node: ASTNode): unknown {
    switch (node.type) {
      ...
      default:
        throw new ExpressionParserError(`[ng-forge: Dynamic Forms] Unknown node type: ${(node as { type: string }).type}`, 0, this.expression);
    }
  }
  ...
  private evaluateMemberAccess(node: { object: ASTNode; property: string }): unknown {
    ...
    if (BLOCKED_PROPERTIES.has(node.property)) {
      throw new ExpressionParserError(`[ng-forge: Dynamic Forms] Property "${node.property}" is not accessible for security reasons`, 0, this.expression);
    }
  ...
  private evaluateBinaryOp(node: { operator: string; left: ASTNode; right: ASTNode }): unknown {
    ...
    switch (node.operator) {
      ...
      default:
        throw new ExpressionParserError(`[ng-forge: Dynamic Forms] Unknown binary operator: ${node.operator}`, 0, this.expression);
    }
  }
  ...
  private evaluateUnaryOp(node: { operator: string; operand: ASTNode }): unknown {
    ...
    switch (node.operator) {
      ...
      default:
        throw new ExpressionParserError(`[ng-forge: Dynamic Forms] Unknown unary operator: ${node.operator}`, 0, this.expression);
    }
  }
  ...
  private evaluateCallExpression(node: { callee: ASTNode; arguments: ASTNode[] }): unknown {
    ...
    if (node.callee.type !== 'MemberAccess') {
      throw new ExpressionParserError('[ng-forge: Dynamic Forms] Only method calls are allowed, not arbitrary function calls', 0, this.expression);
    }
    ...
    if (BLOCKED_PROPERTIES.has(methodName)) {
      throw new ExpressionParserError(`[ng-forge: Dynamic Forms] Property "${methodName}" is not accessible for security reasons`, 0, this.expression);
    }

    // Check if the method is in the whitelist
    if (!this.isMethodSafe(obj, methodName)) {
      throw new ExpressionParserError(`[ng-forge: Dynamic Forms] Method "${methodName}" is not allowed for security reasons`, 0, this.expression);
    }
