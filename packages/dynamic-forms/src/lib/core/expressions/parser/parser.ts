...
export class Parser {
  ...
  parse(): ASTNode {
    ...
    if (this.isAtEnd()) {
      throw new ExpressionParserError('[ng-forge: Dynamic Forms] Empty expression', 0, this.expression);
    }
    ...
    while (true) {
      if (this.match(TokenType.DOT)) {
        ...
        if (!this.check(TokenType.IDENTIFIER)) {
          throw new ExpressionParserError('[ng-forge: Dynamic Forms] Expected property name after "."', this.peek().position, this.expression);
        }
  ...
  private parsePrimary(): ASTNode {
    ...
    throw new ExpressionParserError(`[ng-forge: Dynamic Forms] Unexpected token: ${this.peek().value}`, this.peek().position, this.expression);
  }
  ...
  private consume(type: TokenType, message: string): Token {
    ...
    throw new ExpressionParserError(`[ng-forge: Dynamic Forms] ${message}`, this.peek().position, this.expression);
  }
}
