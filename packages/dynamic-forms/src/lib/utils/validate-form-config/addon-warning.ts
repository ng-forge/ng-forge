...
export type AddonWarning =
  ...
  | { type: 'unknown-type'; fieldKey: string; addonType: string; registeredTypes: readonly string[] }
  | { type: 'type-not-allowed'; fieldKey: string; fieldType: string; addonType: string; allowedTypes: readonly string[] }
  | { type: 'shape-violation'; fieldKey: string; addonType: string; reason: string }
  | { type: 'code-only-type-in-json'; fieldKey: string; addonType: string }
 ...
 * (`type`, `fieldKey`, `addonType`/`slot`/`reason` as relevant) so warnings
 * for different fields or different addon types never collapse to one log
...
export function addonWarningKey(w: AddonWarning): string {
  switch (w.type) {
    ...
    case 'unknown-type':
      return `${w.type}|${w.fieldKey}|${w.addonType}`;
    case 'type-not-allowed':
      return `${w.type}|${w.fieldKey}|${w.fieldType}|${w.addonType}`;
    case 'shape-violation':
      return `${w.type}|${w.fieldKey}|${w.addonType}|${w.reason}`;
    case 'code-only-type-in-json':
      return `${w.type}|${w.fieldKey}|${w.addonType}`;
...
export function formatAddonWarning(w: AddonWarning): string {
  switch (w.type) {
    ...
    case 'unknown-type':
      return (
        `Field '${w.fieldKey}': addon type '${w.addonType}' is not registered. ` +
        `Did you forget withCustomAddon({ type: '${w.addonType}', ... })? ` +
        `Currently registered: ${w.registeredTypes.join(', ') || '(none)'}. Addon dropped.`
      );
    case 'type-not-allowed':
      return (
        `Field '${w.fieldKey}' (type: '${w.fieldType}'): addon type '${w.addonType}' is not in the allowed list. ` +
        `Allowed: ${w.allowedTypes.join(', ')}. Addon dropped.`
      );
    case 'shape-violation':
      return `Field '${w.fieldKey}': addon type '${w.addonType}' shape invalid: ${w.reason}. Addon dropped.`;
    case 'code-only-type-in-json':
      return `Field '${w.fieldKey}': addon type '${w.addonType}' is code-only and cannot be used with JSON-derived configs. Addon dropped.`;
    case 'code-only-action-in-json':
