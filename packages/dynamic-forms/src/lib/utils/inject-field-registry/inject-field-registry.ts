...
export function injectFieldRegistry() {
  ...
  return {
    ...
    async loadTypeComponent(name: string): Promise<Type<unknown>> {
      ...
      if (!fieldType) {
        throw new Error(`[ng-forge: Dynamic Forms] Field type "${name}" is not registered`);
      }
      ...
      if (fieldType.loadComponent) {
        try {
          ...
          throw new Error(`[ng-forge: Dynamic Forms] Failed to load component for field type "${name}": ${error}`);
        }
      }

      throw new Error(`[ng-forge: Dynamic Forms] Field type "${name}" has no component or loadComponent function`);
    },
