...
```bash
npx --yes @ng-forge/dynamic-forms-cli@latest "src/**/*.form.ts" --ui material
```
...
- name: Validate form configs
  run: npx --yes @ng-forge/dynamic-forms-cli@latest "src/**/*.form.ts" --ui material --quiet --require-config
```

Pin the exact version here rather than a tag, so a release cannot change what your pipeline runs.
