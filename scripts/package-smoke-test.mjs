#!/usr/bin/env node
/**
 * Publish-correctness smoke test for a built package.
 *
 * Catches the class of bug where emitted code imports a bare npm specifier
...
if (!DIST) {
  console.error('[smoke] usage: node scripts/package-smoke-test.mjs <dist-dir>');
  process.exit(2);
}

const PKG = join(DIST, 'package.json');
const LABEL = basename(DIST);

if (!existsSync(PKG)) {
  console.error(`[smoke:${LABEL}] ${PKG} not found. Build the project first.`);
  process.exit(2);
}
...
for (const file of files) {
  ...
  try {
    ...
    console.error(`[smoke:${LABEL}] failed to parse ${file}: ${err.message}`);
    process.exit(2);
  }
...
if (offenders.size > 0) {
  console.error(`[smoke:${LABEL}] FAIL: emitted code imports undeclared external dependencies:`);
  for (const [name, where] of offenders) {
    console.error(`  - "${name}" imported in: ${[...where].join(', ')}`);
  }
  console.error(`\nAdd them to the package's "dependencies" (or bundle them).`);
  process.exit(1);
}
