...
\`\`\`bash
npx --yes @ng-forge/dynamic-forms-cli@${version} "path/to/your.form.ts" --ui material
\`\`\`

\`--yes\` matters: without it npx prompts before its first install and waits for
an answer that will never come. The pinned \`@${version}\` matters too: it is the
validator built from the rules below, and unlike a floating tag it cannot change
under you between runs. Requires Node 24 or newer.
...
function ruleCatalogueMd(): string {
  ...
  lines.push(
    ...
    '',
    'Constraints that come from the types are not in this list and cannot be',
    'disabled: the config would not compile either way. A container carrying a',
    '`label`, a select with no `options`, a hidden field with no `value`, a group',
    'with no `fields` and an option written as a bare string are all in that',
    'group. They are still reported, by the same message; there is just no',
    'identifier to switch off.',
    '',
